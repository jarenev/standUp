...
export default function ProfilePage() {
  ...
  const [withdrawItem, setWithdrawItem] = useState<InvItem | null>(null);
  ...
  async function withdraw() {
    ...
    try {
      await api("/api/withdrawals", { inventoryId: withdrawItem.id, pattern: pattern.trim() });
  ...
  return (
    <div className="min-h-screen">
                  ...
                  footer={
                    i.status === "owned" ? (
                      <div className="flex gap-1.5">
                        ...
                        <button onClick={() => { setWithdrawItem(i); setPattern(""); }} className="btn btn-primary flex-1 px-2 py-1.5 text-[11px]">
                          Вывести
                        </button>
                      </div>
                    ) : (
      ...
      {withdrawItem ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 px-4 backdrop-blur-sm" onClick={() => setWithdrawItem(null)}>
          <div className="panel w-full max-w-md p-5" onClick={(e) => e.stopPropagation()}>
            <div className="eyebrow">вывод скина</div>
            <h3 className="display mt-1 text-2xl">ПАТТЕРН</h3>
            <p className="mt-2 text-sm text-slate-400">{withdrawItem.weapon} "{withdrawItem.name}"</p>
            <label className="mt-5 block text-sm font-bold text-slate-300">Введите паттерн</label>
            <input
              autoFocus
              value={pattern}
              onChange={(e) => setPattern(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") void withdraw(); }}
              placeholder="Например: 123"
              maxLength={100}
              className="input mt-2 w-full"
            />
            <div className="mt-2 text-xs text-slate-500">Укажите паттерн именно того предмета, который выводите.</div>
            <div className="mt-5 flex gap-2">
              <button onClick={() => setWithdrawItem(null)} className="btn btn-ghost flex-1 py-2.5">Отмена</button>
              <button onClick={() => void withdraw()} disabled={!pattern.trim()} className="btn btn-primary flex-1 py-2.5 disabled:cursor-not-allowed disabled:opacity-40">Вывести</button>
            </div>
          </div>
        </div>
      ) : null}
